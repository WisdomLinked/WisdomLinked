...
describe("Cancel Event Controller", () => {
  describe("PUT /api/v1/events/:eventId/cancel", () => {
    it("should reject without authentication", async () => {
      const response = await app.handle(
        new Request(
          "http://localhost/api/v1/events/000000000000000000000001/cancel",
          { method: "PUT" }
        )
      );
    ...
    it("should reject if caller is not a participant", async () => {
      ...
      const response = await app.handle(
        new Request(
          `http://localhost/api/v1/events/${event._id.toString()}/cancel`,
          {
            method: "PUT",
    ...
    it("should cancel a pending event", async () => {
      const expert = await createTestUser(
        "expert2",
        "expert2@test.com",
        UserRole.EXPERT
    ...
    it("should cancel an accepted event", async () => {
      const expert = await createTestUser(
        "expert3",
        "expert3@test.com",
        UserRole.EXPERT
    ...
    it("state machine: cannot cancel a completed event", async () => {
      const expert = await createTestUser(
        "expert4",
        "expert4@test.com",
        UserRole.EXPERT
    ...
    it("state machine: cannot cancel a declined event", async () => {
      const expert = await createTestUser(
        "expert5",
        "expert5@test.com",
        UserRole.EXPERT
    ...
    it("admin can cancel any event", async () => {
      const expert = await createTestUser(
        "expert6",
        "expert6@test.com",
        UserRole.EXPERT
