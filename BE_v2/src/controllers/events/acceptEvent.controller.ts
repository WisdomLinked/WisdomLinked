...
export const acceptEventController = new Elysia()
  .use(requireAuth)
  .put(
    "/",
    async (context) => {
      ...
      try {
        ...
        if (event.status !== "pending") {
          set.status = 400;
          return {
            error: `Cannot accept event with status "${event.status}". Only pending events can be accepted.`,
          };
        }
        ...
        if (error instanceof Error) {
          set.status = 500;
          return { error: "Failed to accept event", message: error.message };
        }
        set.status = 500;
        return { error: "Failed to accept event", message: "Unknown error" };
      }
    },
    {
      params: t.Object({
